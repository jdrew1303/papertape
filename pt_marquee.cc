...
/*!
 * \file
 * \brief Main entry point of papertape marquee generator executable.
 *
 */ 
...
#define CHARWIDTH	5		    // width of each character
#define FONTLEN		128		    // number of characters defined in font
#define SPACING		1		    // columns of space between characters


unsigned char font5x7[FONTLEN][CHARWIDTH] = {
...
void PrintHelp(ostream& os) {
    ...
    os << endl;
    os << "Description:" << endl;
    os << "  pt_marquee reads text from stdin and generates output which can be fed to" << endl;
    os << "  pt_render to generate an image of a punched paper tape with human-readable text." << endl;
    os << endl;
    os << "Usage:" << endl;
    os << "  pt_marquee [-v] [-r]"
       << endl;
    ...
    os << "  -r: Rotate text, allowing it to fit on 5-bit-wide tape." << endl;
    os << endl;
}
...
int main(int argc, char **argv)
{
    ...
    bool		Rotate = false;		// rotate text for 5-bit tape
	...
	if (c >= FONTLEN) {
	    // Replace out-of-bounds character with space
	    c = ' ';
	}

	if (Rotate) {
	    // Rotate characters 90 degrees counterclockwise
	    for (n=0; n<7; n++) {
		unsigned char r;
		for (r=0, b=0; b<CHARWIDTH; b++) {
		    if (font5x7[c][b] & (0x40 >> n)) {
			r |= 0x01 << b;
		    }
		}
		data.push_back(r);
	    ...
	    // Normal, non-rotated text
	    for (n=0; n<CHARWIDTH; n++) {
		data.push_back(font5x7[c][n]);
	    }
	}
